...
class SKFChopperModbusInterface(StreamInterface):
    ...
    def any_command(self, command):
        ...
        if len(command[6:]) != length:
            raise ValueError(
                f"Invalid message length, expected {length} but got {len(data)}"
            )

        if function_code == 3:
            return self.handle_read(
                transaction_id, protocol_id, unit, function_code, data
            )
    ...
    def handle_read(self, transaction_id, protocol_id, unit, function_code, data):
        ...
        if isinstance(reply_data, float) and words_to_read == 2:
            ...
            littleendian_bytes = bytearray(
                float_to_raw_bytes(reply_data, low_byte_first=True)
            )
            ...
            if words_to_read == 2:
                ...
                data_length = 2
                reply_data_bytes = int_to_raw_bytes(
                    reply_data, data_length, low_byte_first=False
                )
        else:
